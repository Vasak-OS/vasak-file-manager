...
export const useUserStatsStore = defineStore('userStats', () => {
	...
	const sortedFrequentItems = computed(() => {
		return [...userStats.value.frequentItems].sort(
			(itemA, itemB) => itemB.openCount - itemA.openCount
		);
	});
	...
	async function initStorage() {
		try {
			if (!userStatsStorage.value) {
				userStatsStorage.value = new LazyStore(
	...
	function isFavorite(path: string): boolean {
		return userStats.value.favorites.some((item: FavoriteItem) => item.path === path);
	}
	...
	async function removeFromFavorites(path: string) {
		const favoriteIndex = userStats.value.favorites.findIndex(
			(item: FavoriteItem) => item.path === path
		);
	...
	function getItemTags(path: string): ItemTag[] {
		const taggedItem = userStats.value.taggedItems.find((item: TaggedItem) => item.path === path);

		if (!taggedItem) return [];

		return userStats.value.tags.filter((tag: ItemTag) => taggedItem.tagIds.includes(tag.id));
	}

	async function addTagToItem(path: string, tagId: string, isFile = false) {
		const existingItem = userStats.value.taggedItems.find((item: TaggedItem) => item.path === path);
	...
	async function removeTagFromItem(path: string, tagId: string) {
		...
		if (existingItem) {
			existingItem.tagIds = existingItem.tagIds.filter((id: string) => id !== tagId);

			if (existingItem.tagIds.length === 0) {
				const itemIndex = userStats.value.taggedItems.findIndex(
					(item: TaggedItem) => item.path === path
				);
	...
	async function deleteTag(tagId: string) {
		const tagIndex = userStats.value.tags.findIndex((tag: ItemTag) => tag.id === tagId);

		if (tagIndex !== -1) {
			userStats.value.tags.splice(tagIndex, 1);

			for (const item of userStats.value.taggedItems) {
				item.tagIds = item.tagIds.filter((id: string) => id !== tagId);
			}

			userStats.value.taggedItems = userStats.value.taggedItems.filter(
				(item: TaggedItem) => item.tagIds.length > 0
			);
	...
	async function recordItemOpen(path: string, isFile: boolean) {
		const existingHistoryIndex = userStats.value.history.findIndex(
			(item: HistoryItem) => item.path === path
		);
		...
		const existingFrequent = userStats.value.frequentItems.find(
			(item: FrequentItem) => item.path === path
		);
		...
		if (userStats.value.frequentItems.length > FREQUENT_ITEMS_MAX) {
			userStats.value.frequentItems.sort(
				(itemA: FrequentItem, itemB: FrequentItem) => itemB.openCount - itemA.openCount
			);
	...
	async function removeFromHistory(path: string, openedAt: number) {
		const historyIndex = userStats.value.history.findIndex(
			(item: HistoryItem) => item.path === path && item.openedAt === openedAt
		);
	...
	async function handlePathsDeleted(paths: string[]) {
		...
		userStats.value.favorites = userStats.value.favorites.filter(
			(item: FavoriteItem) => !isPathAffectedByDeletion(item.path, paths)
		);

		userStats.value.taggedItems = userStats.value.taggedItems.filter(
			(item: TaggedItem) => !isPathAffectedByDeletion(item.path, paths)
		);

		userStats.value.history = userStats.value.history.filter(
			(item: HistoryItem) => !isPathAffectedByDeletion(item.path, paths)
		);

		userStats.value.frequentItems = userStats.value.frequentItems.filter(
			(item: FrequentItem) => !isPathAffectedByDeletion(item.path, paths)
		);
	...
	async function removeNonExistentPaths() {
		...
		userStats.value.favorites = userStats.value.favorites.filter(
			(item: FavoriteItem) => !nonExistentPaths.has(item.path)
		);

		userStats.value.taggedItems = userStats.value.taggedItems.filter(
			(item: TaggedItem) => !nonExistentPaths.has(item.path)
		);

		userStats.value.history = userStats.value.history.filter(
			(item: HistoryItem) => !nonExistentPaths.has(item.path)
		);

		userStats.value.frequentItems = userStats.value.frequentItems.filter(
			(item: FrequentItem) => !nonExistentPaths.has(item.path)
		);
