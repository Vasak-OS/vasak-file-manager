import type { Tab, TabAction, TabGroup, Workspace } from '@/types/workspaces';
...
export function parseWorkspaces(workspacesValue: unknown): Workspace[] | null {
	...
	for (const workspaceValue of workspacesValue) {
		...
		const tabGroupIndex = clampIndex(
			getNumber(workspaceValue.currentTabGroupIndex, 0),
			tabGroupMaxIndex
		);
	...
	const primaryWorkspaceIndex = Math.max(
		0,
		parsedWorkspaces.findIndex((workspace) => workspace.isPrimary)
	);
	const currentWorkspaceIndexRaw = parsedWorkspaces.findIndex((workspace) => workspace.isCurrent);
	const currentWorkspaceIndex =
		currentWorkspaceIndexRaw === -1 ? primaryWorkspaceIndex : currentWorkspaceIndexRaw;
...
async function migrateWorkspacesStep(
