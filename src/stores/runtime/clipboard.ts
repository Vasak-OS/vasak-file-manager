...
export const useClipboardStore = defineStore('clipboard', () => {
	...
	const sourceDirectory = computed(() => {
		...
		const allFromSameDir = clipboardItems.value.every(
			(item) =>
				normalizePathForComparison(getParentPath(item.path)) ===
				normalizePathForComparison(firstItemParent)
		);
	...
	function isItemInClipboard(item: DirEntry): boolean {
		return clipboardItems.value.some((clipboardItem) => clipboardItem.path === item.path);
	}
	...
	function isSameAsSourceDirectory(destinationPath: string): boolean {
		...
		return (
			...
			normalizePathForComparison(sourceDirectory.value)
		);
	}
