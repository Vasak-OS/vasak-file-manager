...
export function useFileBrowserSelection(
	...
	function handleEntryMouseUp(entry: DirEntry, event: MouseEvent) {
		...
		const { wasSelected, awaitsSecondClick, lastMouseUpTime, ctrlKey, shiftKey } =
			mouseDownState.value;
		const currentTime = Date.now();
		const timeSinceLastClick = currentTime - lastMouseUpTime;
		const isDoubleClick = awaitsSecondClick && timeSinceLastClick <= 300;
	...
	async function pasteItems(destinationPath?: string): Promise<boolean> {
		...
		if (result.success) {
			...
			if (allSkipped) {
				toastData.value.title = 'notifications.skippedAll';
	...
	async function createNewItem(name: string, itemType: 'directory' | 'file'): Promise<boolean> {
		...
		// biome-ignore lint/suspicious/noControlCharactersInRegex: <Particular Case>
	...
	async function deleteItems(entries: DirEntry[], useTrash: boolean = true): Promise<boolean> {
		...
		try {
			...
			if (result.success) {
				...
				toastData.value.title = useTrash ? 'notifications.trashed' : 'notifications.deleted';
				toastData.value.itemCount = successCount;
				toastData.value.actionText = 'close';
