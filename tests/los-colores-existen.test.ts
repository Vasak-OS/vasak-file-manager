 ...
 *
 * El gestor de archivos tenía 6 así, y las cuatro clases distintas confundían
 * espacios de nombres: `tx-inverted` y `ui-text` eran texto **sobre** un fondo
 * de marca —eso es `tx-on-primary` y `tx-on-secondary`—, `tx-primary` era el
 * color de texto principal —`tx-main`— y `ui-secondary` un borde de foco, que
 * en el resto del repositorio es `focus:border-primary`.
 *
...
describe('las clases de color', () => {
	...
	test('y los tokens que ya no existen no vuelven por su nombre viejo', () => {
		...
		const reemplazos: Record<string, string> = {
			'tx-primary': 'tx-main',
			'tx-inverted': 'tx-on-primary',
			'ui-text': 'tx-on-secondary',
			'ui-secondary': 'primary',
		};
