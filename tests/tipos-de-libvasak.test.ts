...
describe('los tipos de la librería', () => {
	...
	test('y los dos patrones encuentran lo que buscan', async () => {
		...
		try {
			...
			await Bun.write(`${falso}src/paquete.d.ts`, "declare module '@vasakgroup/vue-libvasak' {}\n");
	...
	test('vienen de una versión que los genera', async () => {
		...
		expect(Bun.semver.satisfies(instalada, '>=0.15.1')).toBe(true);
	});
	...
	test.each([
		[
			'UsoDeLaLibreria.vue',
			'una propiedad con el tipo cambiado',
			':title="\'Escritorio\'"',
			':title="42"',
		],
		[
			'UsoDeLaLibreria.vue',
			'una bandera que no es booleana',
			':hide-bar="false"',
			':hide-bar="\'no\'"',
		],
		[
		...
		[
			'UsoDelPropio.vue',
			'una propiedad de un componente propio',
			':etiqueta="\'aceptar\'"',
			':etiqueta="42"',
		],
	])(
		'y se comprueban: %s, %s falla',
		async (archivo, _caso, busca, pone) => {
			...
			try {
				...
				expect(salida).toMatch(
					new RegExp(`${archivo.replace('.', '\\.')}\\(\\d+,\\d+\\): error TS\\d+`)
				);
